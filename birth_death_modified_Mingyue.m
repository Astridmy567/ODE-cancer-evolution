...
function dydt = odesys8_bd(~, y, ...
    ...
    % Space 0
    T0 = H0 + X0 + Y0 + Z0;
    dH0 = (1 - mhx - mhy) * rH_p * H0 - mu * T0 * H0 + fH_p * (H1 - H0);
    dX0 = (1 - mxz) * rX_p * X0 + mhx * rH_p * H0 - mu * T0 * X0 + fX_p * (X1 - X0);
    dY0 = (1 - myz) * rY_p * Y0 + mhy * rH_p * H0 - mu * T0 * Y0 + fY_p * (Y1 - Y0);
    dZ0 = rZ_p * Z0 + mxz * rX_p * X0 + myz * rY_p * Y0 - mu * T0 * Z0 + fZ_p * (Z1 - Z0);

    % Space 1
    T1 = H1 + X1 + Y1 + Z1;
    dH1 = (1 - mhx - mhy) * rH_pp * H1 - mu * T1 * H1 + fH_pp * (H0 - H1);
    dX1 = (1 - mxz) * rX_pp * X1 + mhx * rH_pp * H1 - mu * T1 * X1 + fX_pp * (X0 - X1);
    dY1 = (1 - myz) * rY_pp * Y1 + mhy * rH_pp * H1 - mu * T1 * Y1 + fY_pp * (Y0 - Y1);
    dZ1 = rZ_pp * Z1 + mxz * rX_pp * X1 + myz * rY_pp * Y1 - mu * T1 * Z1 + fZ_pp * (Z0 - Z1);
